...
function PropertyPreview({ selectedLocation }: PropertyPreviewProps) {
  ...
  const { data: properties, isPending } = useGetProperties(queryParams)
  ...
  const getPropertyImage = (property: Property) => {
    ...
    return property.images?.[0]?.url || property.thumbnail || "/modern-property.png"
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-6 text-balance">
            {selectedLocation ? `Properti di ${selectedLocation}` : "Properti Pilihan Terbaik"}
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto text-pretty">
            {selectedLocation
              ? `Temukan berbagai pilihan penginapan terbaik di ${selectedLocation} dengan fasilitas lengkap dan harga terjangkau`
              : "Temukan berbagai pilihan penginapan terbaik dengan fasilitas lengkap dan harga terjangkau di seluruh Indonesia"}
          </p>
        </div>
                          ...
                          {propertyItem.facilities && propertyItem.facilities.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {propertyItem.facilities.slice(0, 2).map((facility, facilityIndex) => (
                                <Badge key={facility.id || facilityIndex} variant="outline" className="text-xs">
                                  {facility.title}
                                </Badge>
                              ))}
